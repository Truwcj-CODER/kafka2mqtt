...
class mqtt_button:
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:
            ...
            line2_stable = curr_line2 == kafka_line2 and curr_line2 == prev_line2
            
            if line2_stable:
                up_pressed = curr_up > prev_up
                down_pressed = curr_down > prev_down
                
                if up_pressed and not down_pressed:
                    print(f"🔘 Button UP được nhấn: {prev_up} → {curr_up}")
                    self.send_kafka(short_addr, 1)  # UP
                elif down_pressed and not up_pressed:
                    print(f"🔘 Button DOWN được nhấn: {prev_down} → {curr_down}")
                    self.send_kafka(short_addr, 0)  # DOWN
                elif up_pressed and down_pressed:
                    print(f"⚠️ Cả UP và DOWN được pressed cùng lúc - Bỏ qua")
                else:
                    print(f"🔁 No change countUp/countDown")
            else:
                print(f"Bạn chờ tí nhé đợi line2 ổn định nè, line2: curr='{curr_line2}', kafka='{kafka_line2}', prev='{prev_line2}'")
